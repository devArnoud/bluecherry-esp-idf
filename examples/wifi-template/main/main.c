...
static esp_err_t wifi_init(const char *ssid, const char *password, wifi_auth_mode_t auth_mode)
{
  ...
  ret = esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_ev_cb, NULL, &wifi_evh);
  if(ret != ESP_OK) {
  ...
  EventBits_t bits = xEventGroupWaitBits(wifi_ev_group, BIT0 | BIT1, pdFALSE, pdFALSE, portMAX_DELAY);
...
void app_main(void)
{
  ESP_LOGI(TAG, "BlueCherry example V1.0.0");
